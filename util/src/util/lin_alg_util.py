...
#
# Copied from tactile_map est_ransac_line.py, written 6 Apr 2015.
#
...
def est_least_sqr_line (pts):
  ...
  # fitLine() returns a ndarray of ndarrays of length 1.
...
# Parameters:
#   pt1, pt2: 1 x 3 NumPy arrays. (pt2 - pt1) is a vector parallel to a line.
#   pts: n x 3 NumPy array. A set of n points.
...
def project_pts_onto_line (pt1, pt2, pts):
  ...
  if line_norm < 1e-6:
    ...
    return np.linalg.norm (pts - pt1, axis=1)
    ...
    #   height will be in terms of origin, i.e. projection onto (pt2 - [0 0 0]).
    # (n x 1) = ((n x 3) * (1 x 3)^T)
    projection = np.dot (pts - pt1, line_unit.T)
